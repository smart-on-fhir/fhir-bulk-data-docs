...
Existing FHIR APIs work well for accessing small amounts of data, but large exports can require hundreds of thousands of requests. These draft specifications outline a standardized, FHIR based approach for exporting bulk data from a FHIR server to a pre-authorized client.  Note that these specifications do not address data compression or data blocking and buffering to optimize transfer rates.
