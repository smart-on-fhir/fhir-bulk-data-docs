...
The decision whether to issue a single access token for use for all three purposes, or to issue multiple tokens is a risk-management decision to be made by the organization responsible for managing access to the resources. To minimize risks associated with token redirection, the scope of each access token SHOULD encompass, and be limited to, the resources requested.  Also, in some cases, the server that receives status queries may be different from the FHIR resource server.  So the use of separate access tokens for requesting FHIR resources and for requesting status information is RECOMMENDED. The scope of resources applicable to the access token required (optionally) for file download will be identical to (or a subset of) the token used for the initial request.  So the authorization server MAY choose to allow the use of the same token for both purposes, or it MAY choose to issue a new token at the time needed for file download.

Regardless of how access tokens are obtained and how many tokens are issued, the requests outlined below MUST include an ```Authorization``` header containing a bearer token received from the responsible authorization server.   Each access token MUST include the identity of each intended recipient (audience).  Upon receipt of a request, the server to whom the token is presented MUST confirm that the token was issued by the applicable authorization server, that the token is intended for that server’s use, that the scope includes the resources requested, and that the time of use is within the valid use period.   If not, the server SHOULD return an invalid_client response.    
